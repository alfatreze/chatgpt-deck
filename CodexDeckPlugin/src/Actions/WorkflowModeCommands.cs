...
public sealed class FastModeCommand : PluginDynamicCommand
{
    ...
    public FastModeCommand() : base("Fast Mode", "Toggle the configured Fast Mode shortcut", "Commands###Core") { }
    protected override void RunCommand(string p) { var r = _adapter.DispatchAsync(new ActionIntent(ProtocolConstants.ActionIntentType, 1, Guid.NewGuid(), "fast_mode_toggle"), CancellationToken.None).GetAwaiter().GetResult(); _status = r.Status; ActionImageChanged(); }
...
{
    private readonly CompanionAdapter _adapter = new(); private string _status = "idle";
    public ContinueNewTaskCommand() : base("Continue in New Task", "Continue using a new Codex task", "Commands###Core") { }
    protected override void RunCommand(string p) { var r = _adapter.DispatchAsync(new ActionIntent(ProtocolConstants.ActionIntentType, 1, Guid.NewGuid(), "continue_new_task"), CancellationToken.None).GetAwaiter().GetResult(); _status = r.Status; ActionImageChanged(); }
