...
| R1-02 | remediation | Reconcile and harden bridge transport | R1-01 | deployed transport, schema, and documentation agree; authenticated server validates actions and serves bounded/concurrent clients | In progress — newline-delimited TCP is documented, clients are concurrent, and reads are bounded to 15 seconds; live integration tests remain |
| R1-03 | remediation | Harden pairing defaults and endpoint publication | R1-02 | macOS defaults to Keychain, plaintext is explicit dev-only, endpoint publish has no permissive secret window | In progress — macOS now defaults to Keychain and endpoint temp files are restricted before publication; automated rotation/denial tests remain |
| R1-04 | remediation | Add real companion integration tests | R1-01, R1-02 | tests exercise live server auth, malformed input, unsupported actions, dispatch states, and reconnect | In progress — live smoke covers hello/auth/unauthorized/unsupported/malformed-id/reconnect; platform dispatch-failure injection remains |
