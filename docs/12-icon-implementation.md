...
| Default action icon | SVG in the packaged `actionicons/` folder, named for the action class | Default artwork applied when an action is added or reset | Applied for the current core, workflow, interrupt, connection, and permissions actions; Focus reset verified it |
...
The temporary Tabler set in `assets/tabler/` uses `external-link`, `target`, `plus`, `player-stop`, `bolt`, `activity`, `plug-connected`, and `shield-check` for the obvious Codex Deck actions. The restored line SVGs are now used as action-picker symbols and default action icons; physical testing confirmed that the stroke-based format renders correctly, including at width 1 and width 2. Resetting Focus Codex to its default restored the packaged target icon, confirming that the `actionicons/` path is recognized. The Loupedeck Icon Library did not show a separate CodexDeck collection after plugin reload, application restart, and service restart; treat browsable library indexing as a host-managed capability distinct from default-icon application. These SVGs are not used as dynamic runtime images.
