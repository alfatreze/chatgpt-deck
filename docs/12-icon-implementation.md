...
| Focus Codex | No custom image | Host default | Host label; action is intentionally simple |
...
| Interrupt Codex (Double Press) | Host default | No custom image currently | Host label plus receipt state |
...
| Test Permissions / Permissions & Connection | Host default currently | Text-led diagnostic feedback; dynamic image treatment remains available for a later pass | Host label plus permission guidance |
